        ...
        try:
            ...
            try:
                ...
                if api.offline_queue.is_online():
                    ...
                    dob_year = _dt.now().year - int(values["Age"])
                    dob = f"{dob_year}-01-01"
                    
                    backend_payload = {
                        "first_name": first,
                        "last_name": " ".join(rest) or "Patient",
                        "gender": gender_map.get(values["Gender"], "UNKNOWN"),
                        "date_of_birth": dob
                    }
                    
                    print(f"[Dashboard] Submitting patient to backend: {backend_payload}")
                    res = api.create_patient(backend_payload)
                    if res.get("status") == "success" or "id" in res:
                        backend_data = res.get("data") or res
                        patient_details["id"] = backend_data.get("id")
                        print(f"✅ Patient created on backend with UUID: {patient_details['id']}")
            except Exception as backend_err:
                print(f"[Dashboard][WARN] Could not create patient on backend: {backend_err}")
